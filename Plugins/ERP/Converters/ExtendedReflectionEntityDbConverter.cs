...
namespace ErpServices.Converters
{
	...
	{
		private readonly NumberListConverter _numberListConverter = new NumberListConverter();
		private readonly IEntityStores _entityStores;
			...
			foreach (var specialTypeProperty in SpecialTypeProperties)
			{
				...
				if (referenceId != null)
				{
					if (specialTypeProperty.PropertyType == typeof(byte[]))
						...
						if (specialTypeProperty.PropertyType.IsEnumerable())
					{
		...
		public override IDictionary<string, object> ConvertTo(T entity)
		{
			...
			foreach (var specialTypeProperty in SpecialTypeProperties)
			{
				...
				if (!propValue.IsDefaultValue(specialTypeProperty.PropertyType))
				{
					...
					{
						var typeElement = specialTypeProperty.PropertyType.GetElementTypeFromEnumerable();
						referenceId = _numberListConverter.ConvertTo(
								from e in (IEnumerable<object>) specialTypeProperty.GetValue(entity, null) 
								select Insert(typeElement, e) );
					}
					else
		...
		object Find(Type type, object referenceId)
		{
			...
			return findEntity.Invoke(store, new object[]
						{
							Convert.ToInt32(referenceId)
						});
		}
		...
		object GetStoreFor(Type t)
		{
			...
			return genericMethod.Invoke(_entityStores, new object[0]);
		}
	}
}