...
namespace ErpServices.Tests
{
	...
	public class EntityStoreTests : ContainerBaseTest
	{
		...
		public void Reload_creates_a_table_with_no_clientId_for_default_store_of_entity_when_clientId_is_empty()
		{
			...
			var entityStore = Container.Resolve<EntityStore<Material>>(new TypedParameter(typeof(string), string.Empty));
			entityStore.Reload();

			db.Received().CreateTable("Material", "Material");
		}
		...
		public void Reload_creates_table_with_client_id_in_name_when_not_empty()
		{
			...
			var entityStore = Container.Resolve<EntityStore<Material>>(new TypedParameter(typeof(string), "myClientIdgoeshereandwild"));
			entityStore.Reload();

			db.Received().CreateTable("Material", "myClientIdgoeshereandwild_Material");
		}
		...
		{
			var db = Container.SubstituteFor<IDatabase>();
			db.GetTable("muchCustom_verystore_Material").Returns(Substitute.For<IDbTable>());
			
			var entityStore = Container.Resolve<EntityStore<Material>>(new TypedParameter(typeof(string), "muchCustom_verystore"));
			entityStore.Reload();

			db.Received(1).GetTable(Arg.Is("muchCustom_verystore_Material"));
			db.DidNotReceiveWithAnyArgs().CreateTable(string.Empty, string.Empty);
		}
