...
namespace ErpServices.Tests
{
	...
	public class XmlDataRowTests : ContainerBaseTest
	{
		...
		[Test]
		[SetCulture("de-DE")]
		public void Update_stores_decimal_values_independent_of_current_culture()
		{
			var xmlFile = Container.SubstituteFor<IFileInfo>();
			xmlFile.FullName.Returns(@"C:\Temp\Test.xml");
			var xmlFileContent = TestData.GetDefaultXmlContent();
			...
			table.Initialize();
			var row = table.CreateDataRow();
			row.Update("desSollADezimalBleibn", (decimal)1234.1234);
			
			Assert.AreEqual("1234.1234", row.Data["desSollADezimalBleibn"]);
		}
		
		[Test]
		public void Update_stores_null_and_empty_string_values_correctly()
		{
			var xmlFile = Container.SubstituteFor<IFileInfo>();
			xmlFile.FullName.Returns(@"C:\Temp\Test.xml");
			var xmlFileContent = TestData.GetDefaultXmlContent();
			...
			table.Initialize();
			var row = table.CreateDataRow();
			row.Update("desSollAnullBleibn", (string)null);
			row.Update("DessollaemptyStringBleibn", string.Empty);

			Assert.IsNull(row.Data["desSollAnullBleibn"]);
			Assert.AreEqual(string.Empty, row.Data["DessollaemptyStringBleibn"]);
		}
