...
namespace ErpServices.Tests
{
	...
	public class XmlDbTableTests : ContainerBaseTest
	{
		...
		[Test]
		[TestCase("Sample.xml", ExpectedResult = "Sample")]
		[TestCase("1C004969-1D2A-4976-A6BD-B821F8F6E448_Sample.xml", ExpectedResult = "1C004969-1D2A-4976-A6BD-B821F8F6E448_Sample")]
		public string Constructor_sets_Filename_to_Xml_FileName_without_extension(string xmlFileName)
		{
			var xmlFile = Container.SubstituteFor<IFileInfo>();
			xmlFile.FullName.Returns(@"C:\Temp\"+xmlFileName);
		...
		[Test]
		public void Initialize_sets_Name_to_table_name_stored_in_loaded_xml_file()
		{
			var xmlFile = Container.SubstituteFor<IFileInfo>();
		...
		[Test]
		public void Initialize_sets_Name_to_passed_name_for_NewXmlDbTables()
		{
			var xmlFile = Container.SubstituteFor<IFileInfo>();
			xmlFile.FullName.Returns(@"C:\Temp\Sample.xml");
			var ms = new MemoryStream();
			xmlFile.Create().Returns(ms);
			
			var table = Container.Resolve<NewXmlDbTable>(new TypedParameter(typeof(string), "Frrraaaaannnnzzz"));
			table.Initialize();
			
			Assert.AreEqual("Frrraaaaannnnzzz", table.Name);
			ms.Dispose();
		}
		...
		[Test]
		public void CreateDataRow_adds_a_new_dataRow_to_the_dataSet()
		{
			var xmlFile = Container.SubstituteFor<IFileInfo>();
		...
		[Test]
		public void Commit_after_adding_a_new_row_table_row_is_written_to_stream()
		{
