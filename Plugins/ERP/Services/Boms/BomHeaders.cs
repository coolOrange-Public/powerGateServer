...
namespace ErpServices.Services.Boms
{
	public class BomHeaders : ErpServiceMethodBase<BomHeader>
	{
		...
		{
			var bomRows = _bomRowsStore.ToArray();
			var items = _itemsStore.ToArray();
			return EntityStore.Select(header => new BomHeader(header)
			{
				Children = bomRows.Where(row => row.ParentNumber == header.Number).Select(row => new BomRow(row)
				{
					Item = items.FirstOrDefault(item => item.Number == row.ChildNumber)
				}).ToList()
			});
		}
