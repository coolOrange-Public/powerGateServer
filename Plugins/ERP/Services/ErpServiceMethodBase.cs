...
namespace ErpServices.Services
{
	public abstract class ErpServiceMethodBase<T> : ServiceMethod<T>
	{
		...
		protected void ThrowElementAlreadyExists<TEntity>(TEntity entity)
		{
			Throw(entity, "A {0} with key: [{1}] already exists!");
		}

		protected void ThrowElementDoesNotExists<TEntity>(TEntity entity)
		{
			Throw(entity, "A {0} with key: [{1}] was not found!");
		}

		protected void Throw<TEntity>(TEntity entity, string message)
		{
