...
namespace SapServices.Tests
{
	...
	public class ExtendedReflectionEntityDbConverterTests : ContainerBaseTest
	{
		...
		public void ConvertTo_converting_a_Type_wih_propertyOfOtherClass_returnsThePrimaryKey_as_data_value_for_complexProperty()
		{
			...
			{
				SomeProp1 = "Heinrich Hitler",
				SomeProp2 = new TestClass2
				{
					SomeProp1 = "Adolf Himmler",
					SomeProp2 = 666
				}
			};
			...
			stores.ResolveFor<TestClass2>().Returns(testClass2Store);
			...
			CollectionAssert.AreEqual(new object[] { "Heinrich Hitler", 999 }, data.Values);
		}

		[Test]
		public void ConvertFrom_Data_with_referenceId_to_other_entityType_returns_instance_where_complexProeprty_is_filled_withEntityWhere_primaryId_is_the_refId()
		{
			var stores = Container.SubstituteFor<IEntityStores>();
			var testClass2Store = Substitute.For<IEntityStore<TestClass2>>();
			...
			Assert.AreEqual("Heinrich Hitler", entity.SomeProp1);
			Assert.AreSame(complexPropInstance, entity.SomeProp2);
		}

		[Test]
		public void ConvertTo_Converting_a_instance_with_empty_ComplexTypeProperty_returns_data_where_complexProeprty_is_null()
		{
			var testInstance = new TestClass
			{
				SomeProp1 = "Heinrich Hitler"
			...
			CollectionAssert.AreEqual(new[] { "SomeProp1", "SomeProp2" }, data.Keys);
			CollectionAssert.AreEqual(new object[] { "Heinrich Hitler", null }, data.Values);
		}
		...
		public void ConvertTo_Converting_a_instance_with_ComplexPropertyList_returns_entity_where_property_contains_list_of_primaryKeys()
		{
			...
			CollectionAssert.AreEqual(new[] { "SomeProp1", "SomeProp2" }, data.Keys);
			CollectionAssert.AreEqual(new object[] { "Heinrich Hitler", "999;888" }, data.Values);
		}

		[Test]
		public void ConvertFrom_data_where_refId_Is_list_of_refIds_returns_entity_where_list_propertyIs_set_withEntites_foreach_refId()
		{
			var stores = Container.SubstituteFor<IEntityStores>();
			var testClass2Store = Substitute.For<IEntityStore<TestClass2>>();
