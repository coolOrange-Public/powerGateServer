...
using coolOrange.Powershell.Cmdlets.Cmdlets.Vault.Facade;
using coolOrange.Powershell.Cmdlets.Cmdlets.Vault.Facade.Files;
...
namespace VaultServices.Tests.Entities.File.FindStrategies
{
	...
	public class FindVaultFilesByPropertiesTests : ContainerBasedTests
	{
		...
		{
			Container.SubstituteFor<cOVaultServices.Vault.IVaultServices>().GetAllPropertyDefinitions("FILE")
		...
		{
			var vault = Container.SubstituteFor<cOVaultServices.Vault.IVaultServices>();
		...
		public void Execute_returns_the_entityFiles_foreach_vaultFile_found_by_the_search()
		{
			...
			var vaultFiles = new[]
			{
				new Autodesk.Connectivity.WebServices.File{Name = "Angela.ipt"},
			...
			CollectionAssert.AreEquivalent(new[] { "Angela.ipt", "Vladimir.ipt", "Barack.ipt" }, findFilesByProperties.Execute().Select(f => f.Name));
		}
