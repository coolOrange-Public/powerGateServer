...
namespace VaultServices.Tests.Entities.File.FindStrategies
{
	...
	public class FindFilesBaseTests : ContainerBasedTests
	{
		...
		private VaultFile CreateVaultFile()
		{
			var vaultFile = Substitute.For<VaultFile>();
			vaultFile.Identifiers.Id.Returns(666);
			vaultFile.Properties.Returns(new Properties
			{
			...
			vaultFile.GetFile().Returns(new Autodesk.Connectivity.WebServices.File
			{
				Name = "Spongebob.iam",
		...
		private IProperty CreateProp(string name, object value)
		{
			return
				new coolOrange.Powershell.Cmdlets.Cmdlets.Vault.Facade.Files.Property(
					new PropertyDefinition(name) { DisplayName = name }, value);
		}
	}
}