...
namespace VaultServices.Tests.Entities.Property.FindStrategies
{
	...
	public abstract class PropertyExpressionParserTests<T> : ExpressionParserTests<T> where T : IBaseObject
	{
		...
		{
			var expression = ExpressionFactory.CreateCallWhere<VaultServices.Entities.Property.Property>(p => p.Value == "Angela&Barack4EverIn<3");
			var propertyExpression = new RequestExpression<VaultServices.Entities.Property.Property>(expression);
			...
			Assert.IsTrue(expressionForEntity.Where.IsSet());
			Assert.AreEqual(1, expressionForEntity.Where.Count());
            Assert.AreEqual(EntityTypeArray + ".Where(e => e.Properties.Any(p => (p.Value == \"Angela&Barack4EverIn<3\")))", expressionForEntity.Base.ToString());
		}
		...
		{
			Expression<Func<VaultServices.Entities.Property.Property, bool>> predicate = p => ((p.Value == null) ? null : (Nullable<bool>)(p.Value.StartsWith("Angela&Barack"))) == (Nullable<bool>)false;
			var expression = ExpressionFactory.CreateCallWhere<VaultServices.Entities.Property.Property>(predicate);
			var propertyExpression = new RequestExpression<VaultServices.Entities.Property.Property>(expression);
			...
			Assert.IsTrue(expressionForEntity.Where.IsSet());
			Assert.AreEqual(1, expressionForEntity.Where.Count());
            Assert.AreEqual(EntityTypeArray + ".Where(e => e.Properties.Any(p => (IIF((p.Value == null), null, Convert(p.Value.StartsWith(\"Angela&Barack\"))) == Convert(False))))", expressionForEntity.Base.ToString());
		}
