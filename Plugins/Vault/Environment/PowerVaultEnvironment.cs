...
using IVaultConnection = cOVaultServices.Vault.IVaultConnection;
...
namespace VaultServices.Environment
{
	public class PowerVaultEnvironment : IVaultEnvironment
	{
		...
		{
			private coolOrange.Powershell.Cmdlets.Cmdlets.Vault.Facade.IVaultConnection _vault;
			internal coolOrange.Powershell.Cmdlets.Cmdlets.Vault.Facade.IVaultConnection Vault
			{
			...
			public IVaultConnection Login(VaultLoginCredentials loginCredentials)
			{
				...
				return new VaultConnection(Vault.Connection);
			}
		}
	}
}